...
describe("User jouney Evals Qwen3 Tests", () => {
  let llmint: LLmint;
  ...
  beforeAll(async () => {
    ...
    llmint = new LLmint(dmr.getLanguageModel());
    html = new HtmlContentGenerator(dmr.getLanguageModel());
  });
  ...
  it("Hello world", async () => {
    ...
    const result = await userjourney.validateIntent.evaluate({
      content: content,
      expectation:
